...
1. **Initialise wiki/ if needed**: `uv run python3 "${CLAUDE_PLUGIN_ROOT}"/skills/wikip/scripts/init.py "<wiki-dir>"`. Creates `pages/papers/`, `pages/concepts/`, `sources/`, `graph.json`, `_schema.json` (typed predicate vocabulary), `index.md` stub.
...
7. **Stage the source doc into the vault**:
   ```bash
   uv run python3 "${CLAUDE_PLUGIN_ROOT}"/skills/wikip/scripts/stage_source.py "<source-dir>" "<wiki-dir>" "<paper-slug>" --title "<title>"
   ```
...
10. **Validate**: `uv run python3 "${CLAUDE_PLUGIN_ROOT}"/skills/wikip/scripts/validate.py "<wiki-dir>"`. Reports broken `[[wiki-links]]`, edges referencing missing pages, predicate type-mismatches (e.g. using `cites` between two concepts), and orphan pages. Regenerates `index.md`. Fix all errors before finishing.
...
12. **Commit the wiki changes.** The wikis repo is a *separate* git repo nested in the project — never `git add wikis/` from the project root. Run:
    ```bash
    uv run python3 "${CLAUDE_PLUGIN_ROOT}"/skills/wikip/scripts/wiki-commit.py -m "feat(corpus): ingest <source-title> into <vault>"
    ```
...
```bash
uv run python3 "${CLAUDE_PLUGIN_ROOT}"/skills/wikip/scripts/merge.py <source-wiki> --into <target-wiki>
```
...
```bash
uv run python3 "${CLAUDE_PLUGIN_ROOT}"/skills/wikip/scripts/audit.py <wiki-dir>
```
