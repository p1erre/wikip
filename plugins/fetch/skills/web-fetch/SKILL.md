...
1. Run the fetcher:
   ```bash
   uv run python3 "${CLAUDE_PLUGIN_ROOT}"/skills/web-fetch/scripts/fetch.py "<url>" --out-dir "<out_dir>"
   ```
...
2. **Run the crawl:**
   ```bash
   uv run python3 "${CLAUDE_PLUGIN_ROOT}"/skills/web-fetch/scripts/crawl.py "<seed-url>" \
     --out-dir "<out_dir>" \
     --path-prefix "<prefix>" \
     --timeout 60
   ```
