...
2. Run:
   ```bash
   uv run python3 "${CLAUDE_PLUGIN_ROOT}"/skills/pdf-extract/scripts/extract.py "<pdf>" --out-dir "<out_dir>"
   ```
...
- **Native PDF with embedded images — misleadingly high quality score (pymupdf layout pollution).** When the detector classifies a PDF as `native` and picks `pymupdf`, the quality score reflects text extraction accuracy only — not layout fidelity. If the PDF contains diagrams with text labels (architecture charts, comparison tables, annotated screenshots), pymupdf reads the label text as standalone prose lines, producing a document where 20–30% of lines are diagram label fragments floating out of context. The quality score can be 0.98+ while the extract is effectively unusable. **Symptom**: many short lines (< 45 chars) that read like UI labels or diagram annotations; zero `![]()` image references in `content.md` despite the PDF being visually image-heavy. **Fix**: re-delete the output directory and re-run with `--force-strategy marker`. Marker understands page layout, keeps label text with its figure, and injects inline image references. The marker quality score will be lower (typically 0.95–0.97) but the extract is far better. **When to apply proactively**: any vendor whitepaper, slide-deck-style PDF, or document whose `pdf_profile.json` shows `images > 0` on multiple pages — do not trust `auto` for these; force marker.
