...
- **work-dir** (optional; default: the host project's bundle area, e.g. `work/<slug>/` — check the project's CLAUDE.md convention)
...
3. Run:
   ```bash
   uv run python3 "${CLAUDE_PLUGIN_ROOT}"/skills/video-transcript-fetch/scripts/fetch.py "<URL>" --out-dir "$OUT" [--lang <lang>]
   ```
...
```bash
uv run python3 "${CLAUDE_PLUGIN_ROOT}"/skills/video-transcript-fetch/scripts/fetch.py --out-dir work/<slug>
```
