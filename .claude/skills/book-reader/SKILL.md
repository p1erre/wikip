...
```bash
uv run python3 .claude/skills/book-reader/scripts/map.py "<book.pdf>"
# optional: --out-dir <dir>   --toc-pages 7-13   --probe-pages 30
```

Each book gets a **self-contained home folder** `work/books/<book-slug>/`. On the
...
on the book's own folder. `work/` is gitignored (large, often copyrighted).
Two outcomes:
...
4. **Finalize:**
   ```bash
   uv run python3 .claude/skills/book-reader/scripts/map.py --build "<out-dir>/draft.json"
   ```
...
uv run python3 .claude/skills/book-reader/scripts/read.py \
